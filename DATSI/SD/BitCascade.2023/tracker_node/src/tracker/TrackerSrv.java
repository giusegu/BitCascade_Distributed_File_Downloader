...
class TrackerSrv extends UnicastRemoteObject implements Tracker  {
    ...
    public synchronized FileInfo lookupFile(String fileName) throws RemoteException {
    	File file = registry.lookup(fileName)
        if (file.exists()) {
            FileInfo fileInfo = new FileInfo();
            fileInfo.setFileName(file.getName());
            fileInfo.setFileSize(file.length());
            // Aquí puedes obtener y establecer otras propiedades de la metainformación del archivo
            return fileInfo;
        } else {
    ...
    static public void main (String args[])  {
        ...
        try {
            ...
            // args[0] = nombre depuracion y arg[1] = puerto del registry 
            Registry registry = LocateRegistry.getRegistry(Integer.parseInt(args[1]));
            registry.rebind(args[0], srv);
